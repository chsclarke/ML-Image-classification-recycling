...
filepath = "/home/pi/Documents/ML-Image-classification-recycling/motion_sensor/picam"
...
def saveImage(settings, width, height, quality, diskSpaceToReserve):
    ...
    filename = filepath + "/" + "image.jpg"
    print("[!!!] Motion detected!")
    t.sleep(3)
   
    os.system("raspistill %s -w %s -h %s -t 200 -e jpg -q %s -n -o %s" % (settings, width, height, quality, filename))#, shell=True)
    
    print("Captured %s" % filename)

    testImage = "python3 ../webserver/testImage.py"
    subprocess.call(testImage, shell=True)
...
while (True):
    ...
    if takePicture:
        lastCapture = time.time()
        #if not os.popen("bash checkEmpty").read():
